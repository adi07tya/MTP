...
api = tweepy.API(auth,wait_on_rate_limit=True)   #.rate_limit_status()
all_tweets=[] 
...
thfd=open(BasePath+"Analysis/JnK_TwitterUsersIDs_3.txt",'r')
flag=0
counttt=0
for handle in thfd:
	counttt+=1
	...
	while len(public_tweets) > 0:
		...
		if(flag==1):
			try:
				...
				print "rate limit status"+api.rate_limit_status()
				pass	
			except tweepy.TweepError as e:
				print e
				pass	
